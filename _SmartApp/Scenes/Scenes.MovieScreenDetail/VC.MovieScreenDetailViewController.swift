//
//  VC.MovieScreenDetailViewController.swift
//  SmartApp
//
...
//
// VC.MovieScreenDetailViewController files are used for:
//  - Received view [var genericView: T!] events and forward then into the viewModel
//  - Iniciate navigation using [var coordinator: MovieScreenDetailCoordinatorProtocol!]
//
// Naming convention: Given a Scene named MovieScreenDetail -> MovieScreenDetailViewController
//

#if canImport(SwiftUI) && DEBUG
// Cant be private/fileprivate
// struct name cant start with _
struct PMovieScreenDetailVCPreviews: PreviewProvider {
    static var previews: some View {
        Common_ViewControllerRepresentable {
            let vc = VC.MovieScreenDetailViewController.instance()!
...
extension VC {
    ...
    //
    // ViewController naming convention: Given a Scene named MovieScreenDetail -> MovieScreenDetailViewController
    //
    
    public class MovieScreenDetailViewController: BaseGenericViewController<V.MovieScreenDetailView>,
                                                         MovieScreenDetailViewControllerProtocol,
        ...
        var viewModel: VM.MovieScreenDetailViewModel?
        var viewModelSetup: VM.MovieScreenDetail.ViewModelInput.InitialSetup?
        var coordinatorActions = GenericObservableObjectForHashable<C.MovieScreenDetailCoordinator.Action>()
        var coordinator: MovieScreenDetailCoordinatorProtocol!
        ...
        public override func rxSetup() {
            ...
            genericView.output.value.sink { [weak self] (result) in
                ...
                case .btnDownloadClicked(movie: let movie):
                    self.viewModel?.send(.handleButtonClicked(movie: movie))
                }
            }.store(in: cancelBag)
        }
        ...
        public override func setupColorsAndStyles() {
            ...
            navigationController.navigationBar.tintColor = ColorSemantic.labelPrimary.uiColor
        }
...
extension VC.MovieScreenDetailViewController: TabBarRootViewControllerProtocol {
