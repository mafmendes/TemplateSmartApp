//
//  VM.MovieScreenDetailViewModel.swift
//  SmartApp
//
...
//
// VM.MovieScreenDetailViewModel files are used for:
...
//
// Naming convention: Given a Scene named MovieScreenDetail -> MovieScreenDetailViewModel
//

// MARK: - MovieScreenDetailViewModel

extension VM {
    
    //
    // ViewModel naming convention: Given a Scene named MovieScreenDetail -> MovieScreenDetailViewModel
    //
    
    struct MovieScreenDetailViewModel: MovieScreenDetailViewModelProtocol,
                                              MVVMGenericViewModelProtocol {
        
        //
        // GenericViewModelProtocol
        //
        typealias ViewModelSetup = VM.MovieScreenDetail.ViewModelInput.InitialSetup
        typealias Action         = VM.MovieScreenDetail.ViewModelInput.Action
        typealias ViewData       = VM.MovieScreenDetail.ViewInput.ViewData
        typealias CoordinatorActions = C.MovieScreenDetailCoordinator.Action
        ...
        #warning("Tutorial: ViewModel dependencies (MUST be resolved at DI.MovieScreenDetailAssembly) ")
...
extension VM.MovieScreenDetailViewModel {
    }
...
extension VM.MovieScreenDetailViewModel {
    ...
    func send(_ action: Action) {
        ...
        case .viewLoaded:
            ...
            fwdStateToView(.loaded(.displayData(
                fullTitle: dataPassedFromPreviousVC.viewModel.fullTitle,
                title: dataPassedFromPreviousVC.viewModel.title,
                imageView: dataPassedFromPreviousVC.viewModel.imageView,
                description: dataPassedFromPreviousVC.viewModel.description,
                imDbRating: dataPassedFromPreviousVC.viewModel.imDbRating,
                metacriticRating: dataPassedFromPreviousVC.viewModel.metacriticRating,
                actors: dataPassedFromPreviousVC.viewModel.actors)))
            } else {
        ...
        case .handle(user: let user, password: let password):
            ...
            sampleUseCase1Protocol
                ...
                .store(in: cancelBag)
        case .handleButtonClicked(movie: let movie):
            present(action: action, some: movie)
        }
    }
...
//                    }
        ...
        case .handleButtonClicked(movie: let movie):
            fwdStateToView(.loaded(.displayAlert(title: movie, type: .noTitle)))
        }
    }
...
private extension VM.MovieScreenDetailViewModel {
