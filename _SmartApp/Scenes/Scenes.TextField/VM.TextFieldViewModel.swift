...
//
// VM.TextFieldViewModel files are used for:
...
//
// Naming convention: Given a Scene named TextField -> TextFieldViewModel
//
...
extension VM {
    ...
    //
    // ViewModel naming convention: Given a Scene named TextField -> TextFieldViewModel
    //
    
    struct TextFieldViewModel: TextFieldViewModelProtocol,
                                              MVVMGenericViewModelProtocol {
        
        //
        // GenericViewModelProtocol
        //
        typealias ViewModelSetup = VM.TextField.ViewModelInput.InitialSetup
        typealias Action         = VM.TextField.ViewModelInput.Action
        typealias ViewData       = VM.TextField.ViewInput.ViewData
        typealias CoordinatorActions = C.TextFieldCoordinator.Action
        ...
        #warning("Tutorial: ViewModel dependencies (MUST be resolved at DI.TextFieldAssembly) ")
...
extension VM.TextFieldViewModel {
    var name: String {
        viewModelSetup?.name ?? ""
    }
}
...
extension VM.TextFieldViewModel {
    ...
    func send(_ action: Action) {
        ...
        case .handle(user: let user, password: let password):
            ...
            sampleUseCase1Protocol
                ...
                .store(in: cancelBag)
        case .handleNewText(newText: let text):
            present(action: action, some: text)
        }
    }
    ...
    func present(action: Action, some: Any?) {
        ...
        case .handleNewText(newText: let text):
            if let model = some as? String {
                fwdStateToView(.loaded(.displayNewLabel(label: model)))
            }
        }
    }
...
private extension VM.TextFieldViewModel {
