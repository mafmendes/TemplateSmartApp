...
//
// C.TitleCoordinator files are used for:
//  - Routing
//
// Naming convention: Given a Scene named Title -> TitleCoordinator
//

extension C {
    final class TitleCoordinator: C.BaseCoordinator, TitleCoordinatorProtocol, GenericSceneCoordinatorProtocol {
        
        var coordinatorIsCompleted: (() -> Void)?
        var cancelBag: CancelBag = CancelBag()
        private(set) weak var viewController: UIViewController?
        var viewControllerProtocol: TitleViewControllerProtocol? {
            if let result = viewController as? TitleViewControllerProtocol {
                return result
            }
            if let navigationController = viewController as? UINavigationController {
                if let result = navigationController.viewControllers.first as? TitleViewControllerProtocol {
                    return result
                }
            }
            return nil
        ...
        func perform(_ action: Action) {
            ...
            case .sampleScreen(let viewModelSetup):
                if let viewModelSetup = viewModelSetup {
                    ...
                    guard let vc = VC.TitleViewController.instance(with: viewModelSetup) else { return }
                    present(viewController: vc, from: viewController, animated: true)
                } else {
                    // New instance with a ViewModel WITHOUT data
                    guard let vc = VC.TitleViewController.instance() else { return }
                    present(viewController: vc, from: viewController, animated: true)
                }
            }
        }
